...
async def get_history(session_id:str)->list[BaseMessage]:
    try:
        data=await redis_client.get(session_key(session_id))
        if not data:
            return []
...
async def save_history(session_id:str, messages:list[BaseMessage])->None:
    try:
        ...
        await redis_client.setex(session_key(session_id), MEMORY_TTL, json.dumps(serialized))
    except Exception as e:
...
async def save_state(session_id:str, state:dict):
    try:
        await redis_client.setex(f"state:{session_id}", MEMORY_TTL, json.dumps(state))
    except Exception as e:
...
async def get_state(session_id:str):
    try:
        data=await redis_client.get(f"state:{session_id}")
